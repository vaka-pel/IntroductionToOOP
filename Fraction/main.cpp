...
public:
	 ...
	 explicit Fraction(int integer)
	{
	 ...
	 Fraction(double decimal) // decimal дес€тична€ дробь
	 {
		 decimal += 1e-10;
		 integer = decimal;  // получаем целую часть дроби
		 decimal -= integer;  //убираем целую часть из дес€тичной дроби
...
std::istream& operator>>(std::istream& is, Fraction& obj)
{
	...
	for (
		...
		numbers[n++] = atoi(pch); // atoi() преобразует строку в число
...
void main()
{
	...
	B = Fraction(8);         // Single argument Constructor создает операцию присваивани€, а оператор присваивани€ просто записывает
